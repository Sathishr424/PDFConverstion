...
function checking(){
    let dataBuffer = fs.readFileSync(path.join(__dirname, 'input.pdf'));
    ...
    pdf(dataBuffer).then(function(data_text) {
        ...
        console.log(text);
        var str = ""
        var start = data_text.text.match()
        var x = data_text.text
        var ret_data = {
            date: text[6],
            r_date: text[9].match(/\d[0-9/]+/)[0],
